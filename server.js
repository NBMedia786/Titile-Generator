...
const MODEL_PRIMARY   = process.env.MODEL || 'gemini-2.5-pro';
const MODEL_FALLBACKS = String(process.env.MODEL_FALLBACKS || 'gemini-2.5-pro')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
const MODEL_PRIORITY  = [...new Set([MODEL_PRIMARY, ...MODEL_FALLBACKS])];
...
app.post('/api/generate', queuedRouteWithSSE(async (req, res) => {
  try {
    ...
    const modelNames = MODEL_PRIORITY.length ? MODEL_PRIORITY : [MODEL_PRIMARY];
    let result = null;
    let modelUsed = null;
    let lastError = null;
    ...
    for (const modelName of modelNames) {
      try {
        req._queueProgress?.(30, `model warmup (${modelName})`);
        ...
        req._queueProgress?.(55, `analyzing video (${modelName})`);
        ...
        const retryable = /invalid argument/.test(msg) || /model.*not found/.test(msg) || /does not support/.test(msg);

        console.warn(`⚠️ Model "${modelName}" failed: ${err?.message || err}`);

        if (!retryable || modelName === modelNames[modelNames.length - 1]) {
          throw err;
        }

        req._queueProgress?.(35, `model fallback (${modelName} failed, trying next)`); // minimal feedback
      }
    }

    if (!result) {
      throw lastError || new Error('Generation failed before receiving any model response.');
    }
    ...
    const saved = historyStore.save({
      ...
      extraMeta: {
        meta,
        input: { videoSource, topic, titleHint, contextText, displayName, playback, modelUsed }
      }
    });
